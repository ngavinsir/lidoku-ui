...
<div class={clsx(className, 'flex flex-col space-y-10 sm:space-y-5')}>
	...
	<!-- Remove icon
		<IconButton label="Remove Number" on:click={() => board.removeSelectedCell()}>
			<svg
				class="w-6 h-6"
				fill="none"
				stroke="currentColor"
	...
	<!-- Notes icon -->
	<!-- <IconButton label="Manual Notes" toggleable={true} on:click={() => board.toggleTakingNotes()}>
			<svg
				class="w-6 h-6"
				fill="none"
				stroke="currentColor"
	...
	<NumberSelector on:select-number />
</div>
